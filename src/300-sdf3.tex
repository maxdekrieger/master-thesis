...
Expr(
  ...
  \section{\label{sec:webdsl-grammar}WebDSL Grammar Specification}
    ...
    List various constructs: file-level unit, top-level declarations that are re-used in other contexts, many injections, deprecated parts of the syntax but still mainainted for backwards compatibility reasons, some productions added for the sake of syntactic autocompletion.

    Reason to switch to SDF3:
    \\- SDF2 is deprecated
    \\- Good case study for performance of SDF3
    \\- Good case study for disambiguation without prefer/avoid 
