...
rules
  ...
  stringPartOk(s, InterpValue(InterpSimpleExp(simple_exp))) :- { T }
    ...
    \subsection{\label{subsec:simple-variables}Variables}
      ...
      Similar to other imperative languages, WebDSL allows the use of variables to store values. These variables can be defined on multiple levels, such as in the module, within a function or at the top of a page/template definition. Additionally, functions may be embedded in entities, allowing direct access to entity properties as variables without the intervention of ``\texttt{this}''.
