  ...
  stmtOk(s, VarDeclInit(x, sort, exp), _) :- { sortType expType }
    ...
    @x.type := t.
      ...
      \subsubsection{Typing of the addition expression}
        ...
        To calculate the return type of addition, we introduce a functional rule that calculates the least-upper-bound of two types: \texttt{lubForAdd : TYPE * TYPE -> TYPE}.

        \begin{itemize}
          \item [\textbf{TO-DO:}]
          \item Give Statix rule and examples of the LUB
        \end{itemize}
...
module x
  ...
  module m
    entity Person { 
      ...
      \begin{itemize}
        \item [\textbf{TO-DO:}]
        \item Special properties (Derived, \texttt{name})
        \item Instantiation
      \end{itemize}
    ...
    ~s
        ...
        \begin{subfigure}[b]{0.7\textwidth}
          ...
          \begin{tikzpicture}
            ...
            \draw[decl edge] (s_m.east) -- (t_p.west) node[midway, sloped, fill=white] {type};
            \draw[decl edge] (s_m.east) -- (t_t.west) node[midway, sloped, fill=white] {type};
          \end{tikzpicture}
