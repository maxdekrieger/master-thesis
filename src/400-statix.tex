...
module x
  ...
  module m
    entity Person { 
      ...
      To prevent duplicate entity definitions, the \texttt{declareType} rule as originally shown in \cref{fig:webdsl-basics-built-in-types-decl} is extended with one additional rule as shown in \cref{fig:webdsl-entity-prevent-duplicates}. \textbf{Line 4} was added to \texttt{declareType}, to make sure when you declare a new type or entity, its name is unique.
  ...
  rules
    ...
    declareType(s, name, t) :-
      ...
      resolveType(s, name) == [(_, (_, t))] | error $[Type [name] is defined multiple times] @name.
        \end{minted}
        \caption{\label{fig:webdsl-entity-prevent-duplicates}\texttt{declareType} shows an error when two types with the same name are declared}
      \end{figure}
