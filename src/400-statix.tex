...
rules
  ...
  importedByApplicationRoot(s_global, s_mod, mod) :- {s_app}
    ...
    \subsection{\label{subsec:revised-module-system}Revised WebDSL Module System}
      ...
      Although all revised module systems would massively boost the performance of the WebDSL Statix specification, taking longer than a minute for a real world application is not acceptable for a static analysis. To this end, we need to speed up the analysis time even more. For other performance boosts as we explain in the next section, a scope graph representation with split modules was required. Since the wildcard import system is more convenient for WebDSL developers, the WebDSL Statix specification is developped with the revised module system that uses wildcard imports.
  ...
  \section{\label{sec:built-in-library}Pre-analyzed built-in library}
    ...
    \textbf{TO-DO: Show speedup, discuss results and discuss difference in timings with previous section due to the evaluation happening at a later stage}
