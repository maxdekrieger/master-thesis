...
template outputFortyTwo {
  ...
  \section{\label{sec:current-implementation}Current Implementation of WebDSL}
    ...
    In the paper where Visser first introduced WebDSL (\citeyear{Visser2007}), it is presented as a large case study in domain-specific language engineering. Visser describes the implementation of the language with its syntax definition in SDF2 and the code generation by term transformation in Stratego. The core of the current implementation is still the same, 15 years later. While the language has matured and is extended in many useful ways, the syntax is still defined in SDF2 and the rest of the compilation chain is implemented in Stratego. SDF2 is the successor of the first version of the Syntax Definition Formalism SDF \autocite{HeeringHKR89,Visser97}.
    
    An application in WebDSL is compiled to a Java code and a persisted data model. Before the correct target code can be generated, the WebDSL code has to pass the compilation chain that exists of components such as the parser, desugaring, typechecking. We describe the parser generated from the WebDSL SDF2 definition next chapter (\cref{chap:sdf3}) and in the rest of this section we discuss the implementation of the WebDSL compiler in Stratego.
    ...
    \subsection{WebDSL in Stratego}
      ...
      Paragraph about Stratego/XT, modern Spoofax with different meta languages, boast about Spoofax.
